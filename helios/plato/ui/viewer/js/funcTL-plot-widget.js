...
class FuncTLPlotWidget extends BaseWidget {
    constructor(el, id) {
        super(el, id, 250)
        ...
        this.cmgr = new FuncColorManager
    ...
    on_update_data(json) {
        console.log(json)
        console.log(json.processorSpecific.grids[0])
        // if (json == null) {
        //     this.series_time = null
        //     this.series = {}
    ...
    _update_plot(data) {
        ...
        Plotly.react(this.plot_div[0], data, this.layout, {displaylogo: false, modeBarButtonsToRemove: FuncTLPlotWidget.plotly_buttons_to_remove, scrollZoom: true})
    ...
    on_render() {
        ...
        const width = this.jbody.width(), height = this.jbody.height()
        const camera = new THREE.OrthographicCamera( width/-2, width/2, height/2, height/-2, -100, 100 );

        const renderer = new THREE.WebGLRenderer();
        renderer.setSize(this.jbody.width(), this.jbody.height());
        this.plot_div[0].append( renderer.domElement );

        const geometry = new THREE.PlaneGeometry( 10, 200);
        const material = new THREE.MeshBasicMaterial( { color: 0x000000 } );
        const cube = new THREE.Mesh( geometry, material );
        cube.position.set(100, 0);
        scene.add( cube );
...
FuncTLPlotWidget.plotly_buttons_to_remove = ['editInChartStudio','sendDataToCloud','lasso2d','zoomIn2d','autoScale2d',]
FuncTLPlotWidget.typename = 'funcTL-plot'
FuncTLPlotWidget.description = 'A function call stack plot'
FuncTLPlotWidget.processor_type = Processors.FUNCTL
FuncTLPlotWidget.data_type = DataTypes.FUNCTL
